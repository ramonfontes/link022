...
def topology():
    ...
    info("*** Adding Link\n")
    ap1.setMasterMode(intf='ap1-wlan0', ssid='ap1-ssid',
                      channel='1', mode='n')
    net.addLink(ap1, s1)
    net.addLink(c1, s1)
    ...
    net.build()
    wan0 = net.addNAT(name='wan0', linkTo='s1').configDefault()
    s1.start([])
    ...
    makeTerm(ap1, title='agent', cmd="bash -c 'echo \"running agent...\" "
                                     "&& ../binary/link022_agent "
                                     "-ca=../demo/cert/tls_cert_key/server/ca.crt "
    ...
    makeTerm(net.hosts[1], title='prom', cmd="bash -c 'echo \"running server...\" "
                                             "&& ../agent/monitoring/prometheus/exposition_server "
                                             "-ca ../demo/cert/tls_cert_key/client/ca.crt "
                                             "-cert ../demo/cert/tls_cert_key/client/client.crt "
                                             "-key ../demo/cert/tls_cert_key/client/client.key ""-target_name www.example.com "
                                             "-target_addr 10.0.0.1:10162 "
                                             "-listen_addr 10.0.0.4:8080;'")
